...
for /r %input% %%i in (*.material.bin) do (
	...
	java -jar MaterialBinTool-0.8.2-all.jar -u .\%%~nxi -o .\src\
	if not exist src\ (
		set /a n-=1
		echo Error.
		del %%~nxi
	...
	for /d %%f in (*) do (
		...
		for /f "delims=*" %%d in (%%f\%%f.json) do (
			set /a l+=1
			if "%%d" == "  "formatVersion": "V1"," (
				echo   "formatVersion": "V2",>> tmp.json
			) else (
				echo %%d>> tmp.json
			)
		)
	...
	cd ..	
	java -jar MaterialBinTool-0.9.0-unofficial.jar -r .\src\* -o .\ 
	echo OK.
	copy /y %%~nxi %%i >nul
	del %%~nxi
	rd /s /q .\src\
